...
export default function CourseList() {
	...
	const mappedCourses = Array.isArray(courses) ? courses.map(c => {
		...
		return {
			...
			rating: 4.8,
			image: getCourseImage(c.category),
		...
		<div className="flex bg-white min-h-screen font-sans antialiased text-gray-800">
			...
			<div className="flex flex-col flex-1 min-h-screen">
				...
				<div className="flex flex-col flex-1 p-8 gap-8 max-w-[1200px] w-full mx-auto">
					...
					<div className="bg-gradient-to-r from-[#7C52AA] via-[#9F52AA] to-[#E040A0] rounded-[40px] p-8 text-white flex justify-between items-center shadow-lg relative overflow-hidden">
						<div className="flex flex-col items-start gap-3 z-10 max-w-[60%]">
						...
						<div className="flex gap-2">
							{["All Courses", "Design", "Tech", "Business"].map((tab) => (
								<button
									...
									className={`py-2 px-6 rounded-full font-bold text-sm transition border-0 ${
										activeTab === tab 
					...
					<div className="grid grid-cols-3 gap-6 w-full">
						{sortedCourses.map((course) => (
							<div key={course.id} className="flex flex-col bg-white rounded-[32px] border border-solid border-[#DCC8E088] shadow-sm hover:shadow-md transition overflow-hidden">
								{/* Thumbnail Cover */}
